...
export const UpcomingReminders = () => {
  const { reminders, toggleReminderCompletion, openQuickAdd } = useProductivity();
  const pendingReminders = reminders.filter((r) => r.status !== 'Completed').slice(0, 4);
    ...
    <div className="p-6 rounded-3xl glass-panel border border-white/10 flex flex-col h-full">
      <div className="flex items-center justify-between pb-3 border-b border-white/10">
        <div>
          ...
          <p className="text-xs text-slate-400">Scheduled alerts & upcoming tasks</p>
        </div>
      ...
      <div className="flex-1 mt-4 space-y-2.5 max-h-72 overflow-y-auto pr-1">
        {pendingReminders.length === 0 ? (
          <div className="py-10 text-center text-slate-400 text-xs flex flex-col items-center justify-center gap-2.5">
            ...
            <div>
              <p className="font-bold text-slate-200 text-sm">No upcoming reminders 🔔</p>
              <p className="text-slate-400 text-xs mt-0.5">Set a time-sensitive alert.</p>
            </div>
            <button
              onClick={() => openQuickAdd('reminder')}
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-xs font-bold text-slate-100 truncate">{rem.title}</p>
